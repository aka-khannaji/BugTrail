...
- **Commit and push to origin only.** The user owns upstream (origin = https://github.com/aka-khannaji/BugTrail). Pushing is allowed; never force-push or rewrite shared history.
...
- CI/Action files: `.github/workflows/ci.yml` (test matrix, microservice lint, docker build+health), `action.yml` (reusable `bugtrail investigate` action), `.github/workflows/bug-investigation.yml` (example issue workflow). Public URLs use the user's real repo `aka-khannaji/BugTrail`.
