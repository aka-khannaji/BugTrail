...
- **Bug zoo:** 4 fixtures — Python (`order_service`), JS/V8 (`cart_service`), PHP/Laravel (`order_placement`, deploy-root paths), and a reformat-masked low-confidence case (`checkout_discount`, now resolved deterministically by diff analysis). Live demo ranked the retry commit **#1 at 97%** deterministically.
