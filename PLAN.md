...
- **Drop-in packaging:** hatchling build; `pip install .` works; published on PyPI as **`getbugtrail`** (the bare `bugtrail` name is blocked by a similar-name rule against the existing `bug-trail`). Console script asserted in CI. Root `README.md` + Quickstart.
